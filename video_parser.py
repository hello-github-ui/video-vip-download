...
class VideoParser:
    ...
    def open_in_browser(self, url, inject_download=True):
        """
        使用 Playwright 在浏览器中打开解析链接，并可选注入下载按钮
        如果浏览器已打开，则复用现有浏览器

        Args:
            url (str): 解析接口URL
            inject_download (bool): 是否注入下载按钮
        ...
        try:
            if self._playwright_browser is None or self._playwright_browser.is_closed():
            ...
            if self._playwright_page is None or self._playwright_page.is_closed():
                self._playwright_page = self._playwright_context.new_page()

            self._playwright_page.goto(url, wait_until='domcontentloaded')

            if inject_download:
                self._inject_download_button()
    ...
    def _inject_download_button(self):
        """
        在当前页面注入下载按钮
        """
        ...
        try:
            self._playwright_page.evaluate(download_js)
            print("已注入下载按钮")
        except Exception as e:
            print(f"注入下载按钮失败: {str(e)}")
